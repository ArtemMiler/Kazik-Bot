...
from ..Keyboards.gamemode import start_keyboard
from Logic import BALANCE
from ..additional_functoins import send_game
...
async def demo_button_handler(callback_query: CallbackQuery, state: FSMContext):
    ...
    await send_game(callback_query.message, state, BALANCE)
    await callback_query.answer()
...
async def cmd_help(message):
    await message.answer(
        "🃏<b>JOKER CASINO</b>🃏 поддерживает режим игры только с игровой валютой на данный момент\n\n"
        "Для выбора режима нажмите <b>/start</b>\n"
        "(<i>В группах доступен только демо-счет</i>)\n\n"
        "После этого для игры используйте <b>/spin</b>\n"
        ...
        "В соответствии с этой механикой он будет учитываться по собственной цене <b>только</b> в линии из 5 элементов.\n\n"
        "<b>💎 - Bonus</b> символ сам по себе в комбинациях не участвует и не имеет никакой стоимости, "
